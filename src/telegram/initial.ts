...
import {
  referralLastTxnAction,
  referralStatsAction,
  referralStatsHandler,
  referralTotalCountAction,
...
export function telegramBotLaunch() {
  ...
  Bot.on('inline_query', async (query) => {
    ...
    const startappLink = `https://t.me/${
      process.env.TELEGRAM_BOT_NAME
    }/vtester?startapp=inviterId_${String(query.from.id)}`;
  ...
  Bot.on('callback_query', async (query: TelegramBot.CallbackQuery) => {
    ...
    switch (true) {
      ...
      case query.data.indexOf('tourtakepart') > -1:
        await tournamentTakePartHandler(
          Bot,
          query,
          Number(query.data.replace('tourtakepart', '')),
        );
        break;
      ...
      case query.data.indexOf('duelcancel') > -1:
        ...
        break;
      case query.data.indexOf('dailyReward') > -1:
        await dailyRewardHandler(Bot, query);
        break;
    }
  });
