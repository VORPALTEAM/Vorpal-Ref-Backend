...
export async function getUserInviterByTelegramId(
...
export async function writeReferralStats(data: ReferralTransactionData) {
  ...
  const result = await runQueryWithParams(
    query,
    [data.to, data.for, data.resource, data.amount, dt, data.level],
    false,
  );
...
export async function getReferralCount(
  inviterId: number,
): Promise<{ level1: number; level2: number }> {
...
export async function getReferralTotalRewardsById(
  userId: number,
...
export async function getReferralTotalRewardsByUser(
  login: string,
): Promise<{ item: string; amount: number }[]> {
  const user = await getUserData(login);
  if (!user) return [];
...
export async function getUserReferralLink(
  ...
  const query = `
    ...
    VALUES ($1, $2)
    ON CONFLICT (user_id, link) 
    DO UPDATE SET link = EXCLUDED.link
    RETURNING link;`;
  const result = await runQueryWithParams(query, [userId, newLink], true);
  return result && result.length > 0 ? result[0].link : null;
}
...
export async function getUserByReferralLink(
  ...
  const query = `
   ...
   WHERE link = $1;
   `;
  const result = await runQueryWithParams(query, [link], true);
